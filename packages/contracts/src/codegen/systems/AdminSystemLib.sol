...
library AdminSystemLib {
  ...
  function setBlueprintChunk(AdminSystemType self, Vec3 chunkCoord, bytes memory chunkData) internal {
    return CallWrapper(self.toResourceId(), address(0)).setBlueprintChunk(chunkCoord, chunkData);
  }
  ...
  function setBlueprintChunk(CallWrapper memory self, Vec3 chunkCoord, bytes memory chunkData) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert AdminSystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(_setBlueprintChunk_Vec3_bytes.setBlueprintChunk, (chunkCoord, chunkData));
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }
  ...
  function setBlueprintChunk(RootCallWrapper memory self, Vec3 chunkCoord, bytes memory chunkData) internal {
    bytes memory systemCall = abi.encodeCall(_setBlueprintChunk_Vec3_bytes.setBlueprintChunk, (chunkCoord, chunkData));
    SystemCall.callWithHooksOrRevert(self.from, self.systemId, systemCall, msg.value);
  }
...
interface _setBlueprintChunk_Vec3_bytes {
  function setBlueprintChunk(Vec3 chunkCoord, bytes memory chunkData) external;
}
